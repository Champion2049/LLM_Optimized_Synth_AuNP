...
# ==============================================================================
# --- DCN Model Components ---
# ==============================================================================
class CrossNetwork(nn.Module):
    def __init__(self, input_dim, num_layers):
        super(CrossNetwork, self).__init__()
        self.num_layers = num_layers
        self.kernels = nn.Parameter(torch.Tensor(num_layers, input_dim, 1))
        self.biases = nn.Parameter(torch.Tensor(num_layers, input_dim))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            xl_w = torch.matmul(xl.unsqueeze(1), self.kernels[i]).squeeze(1)
            xl = torch.matmul(x0, xl_w.unsqueeze(2)).squeeze(2) + self.biases[i] + xl
        return xl
...
class DeepNetwork(nn.Module):
    def __init__(self, input_dim, trial):
        ...
        layers = []
        n_layers = trial.suggest_int('deep_n_layers', 2, 5)
        in_dim = input_dim
        for i in range(n_layers):
            out_dim = trial.suggest_int(f'deep_n_units_l{i}', 64, 512, step=64)
            layers.append(nn.Linear(in_dim, out_dim))
            layers.append(nn.BatchNorm1d(out_dim))
            layers.append(nn.ReLU())
            layers.append(nn.Dropout(trial.suggest_float(f'deep_dropout_l{i}', 0.1, 0.5)))
            in_dim = out_dim
        self.mlp = nn.Sequential(*layers)
        self.output_dim = in_dim # Store the output dimension
...
class DCN(nn.Module):
    def __init__(self, input_dim, reg_output_dim, trial):
        ...
        self.cross_net = CrossNetwork(input_dim, trial.suggest_int('cross_layers', 1, 4))
        self.deep_net = DeepNetwork(input_dim, trial)
        
        # Concatenated dimension
        concat_dim = input_dim + self.deep_net.output_dim
        
        # Regression Head
        self.regression_head = nn.Linear(concat_dim, reg_output_dim)
        
        # Classification Head
        self.classification_head = nn.Linear(concat_dim, 1)
    ...
    def forward(self, x):
        ...
        reg_output = self.regression_head(concat_output)
        class_output = self.classification_head(concat_output)
...
# ==============================================================================
def objective(trial):
    # Using DCN model for the objective function
    model = DCN(X_train_processed.shape[1], len(REGRESSION_TARGETS), trial).to(device)
...
# --- Run Optuna Study ---
print("Starting hyperparameter optimization for DCN...")
...
# ==============================================================================
print("\nTraining final DCN model with best hyperparameters...")
final_model = DCN(X_train_processed.shape[1], len(REGRESSION_TARGETS), study.best_trial).to(device)
...
model_path = os.path.join(MODEL_DIR, f"dcn_model_{timestamp}.pth")
torch.save(final_model.state_dict(), model_path)
joblib.dump(feature_preprocessor, os.path.join(MODEL_DIR, f"feature_preprocessor_dcn_{timestamp}.joblib"))
joblib.dump(scaler_y_reg, os.path.join(MODEL_DIR, f"regression_scaler_dcn_{timestamp}.joblib"))
print(f"Final DCN model and preprocessors saved with timestamp: {timestamp}")
...
print("\n--- Classification Performance (DCN) ---")
print(classification_report(y_val_binary_class, preds_binary_class, target_names=['Unsuccessful', 'Successful']))

# --- Plotting ---
# R² Score Plot
plt.figure(figsize=(10, 5))
plt.bar(REGRESSION_TARGETS, r2_scores, color='teal')
plt.title('R² Score per Regression Target (DCN)')
plt.ylabel('R² Score')
plt.xticks(rotation=15, ha="right")
plt.tight_layout()
plt.savefig(os.path.join(MODEL_DIR, f"r2_scores_dcn_{timestamp}.png"))
plt.show()
...
plt.title("Confusion Matrix (DCN)")
plt.savefig(os.path.join(MODEL_DIR, f"confusion_matrix_dcn_{timestamp}.png"))
plt.show()
